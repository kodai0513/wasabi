...
impl fmt::Write for VramTextWriter<'_> {
    fn write_str(&mut self, s: &str) -> fmt::Result {
        for c in s.chars() {
            ...
            draw_str_fg(self.vram, self.cursor_x, self.cursor_y, 0xffffff, c.encode_utf8(&mut buf));
            self.cursor_x += 8;
        }
        Ok(())
    }
...
// 定数
const EIF_GRAPHICS_OUTPUT_PROTOCOL_GUID: EfiGuid = EfiGuid {
    data0: 0x9042a9de,
    data1: 0x23dc,
    data2: 0x4a38,
...
fn locate_graphic_protocol<'a>(
    ...
    let status: EfiStatus = (efi_system_table.boot_services.locate_protocol)(
        &EIF_GRAPHICS_OUTPUT_PROTOCOL_GUID,
        null_mut::<EfiVoid>(),
